...
-- ----------------------------------------------------------------------------
-- Jede Person markiert pro Tag, ob sie "da" ist oder "theoretisch könnte".
--   status = 'available' (sicher da) | 'maybe' (könnte theoretisch)
...
create table if not exists public.availability (
  ...
  day        date not null,
  status     text not null check (status in ('available', 'maybe')),
  created_at timestamptz not null default now(),
  primary key (user_id, day)
);

create index if not exists availability_day_idx on public.availability (day);
comment on table public.availability is 'Tagesverfügbarkeit pro Person: available = da, maybe = könnte.';
